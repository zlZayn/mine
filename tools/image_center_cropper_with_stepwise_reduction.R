...
crop <- function(width_step_pixel, input_image_path, crop_aspect_ratio=1/1, min_size_ratio=0.1, min_pixel_limit=100) {
  # 最终目的/动机
  ...
  #   Notice: This tool is particularly suitable for images where the main subject is located in the central area # nolint
  ...
  output_directory <- paste0(tools::file_path_sans_ext(input_image_path), "_output")
  dir.create(output_directory, showWarnings = FALSE, recursive = TRUE)
  
  # 读取图像/Read image
  source_image <- image_read(input_image_path)
  source_width_pixel <- image_info(source_image)$width
  source_height_pixel <- image_info(source_image)$height
  ...
  if (source_width_pixel / source_height_pixel > crop_aspect_ratio) {
    ...
    current_crop_width_pixel <- as.integer(source_height_pixel * crop_aspect_ratio)
  } else {
    current_crop_width_pixel <- source_width_pixel
    current_crop_height_pixel <- as.integer(source_width_pixel / crop_aspect_ratio)
  }
  ...
  original_file_name <- tools::file_path_sans_ext(basename(input_image_path))
  file_extension <- tools::file_ext(input_image_path)
  ...
  while (TRUE) {
    ...
    if (current_crop_width_pixel < min_allowed_crop_width_pixel || 
        current_crop_height_pixel < min_allowed_crop_height_pixel ||
        relative_area_ratio < min_size_ratio) {
      reasons_zh <- c()
      reasons_en <- c()
      if (current_crop_width_pixel < min_allowed_crop_width_pixel || current_crop_height_pixel < min_allowed_crop_height_pixel) {
        reasons_zh <- c(reasons_zh, paste0("宽/高限制(宽=", current_crop_width_pixel, ", 高=", current_crop_height_pixel, ") < 最小允许值(宽=", min_allowed_crop_width_pixel, ", 高=", min_allowed_crop_height_pixel, ")"))
        reasons_en <- c(reasons_en, paste0("Width/height limit(width=", current_crop_width_pixel, ", height=", current_crop_height_pixel, ") < min allowed values(width=", min_allowed_crop_width_pixel, ", height=", min_allowed_crop_height_pixel, ")"))
      }
      if (relative_area_ratio < min_size_ratio) {
        reasons_zh <- c(reasons_zh, paste0("面积比例限制(当前比例=", relative_area_ratio, ") < 最小比例(", min_size_ratio, ")"))
        reasons_en <- c(reasons_en, paste0("Area ratio limit(current ratio=", relative_area_ratio, ") < min ratio(", min_size_ratio, ")"))
      }
      
      if (length(reasons_zh) > 0 && length(reasons_en) > 0) {
        cat(paste0("停止生成图像 ", crop_sequence_index, "，原因：", paste(reasons_zh, collapse = ", "), "\n"))
        cat(paste0("Stop generating image ", crop_sequence_index, ", reasons: ", paste(reasons_en, collapse = ", "), "\n"))
      }
      break
    }
    ...
    cropped_image <- image_crop(
      source_image,
      geometry_area(
        width = current_crop_width_pixel,
        height = current_crop_height_pixel,
        x_off = crop_left_position_pixel,
        y_off = crop_top_position_pixel
      )
    )
    
    # 保存图像/Save image
    output_file_name <- paste0(
      output_directory, "/", original_file_name, "_crop", crop_sequence_index, "_",
      current_crop_width_pixel, "_", current_crop_height_pixel, "_", relative_area_ratio, ".", tolower(file_extension)
    )
    image_write(cropped_image, path = output_file_name)
    
    # 计算下一步尺寸/Calculate next dimensions
    next_crop_width_pixel <- current_crop_width_pixel - width_step_pixel
    next_crop_height_pixel <- round(next_crop_width_pixel / crop_aspect_ratio)
    next_crop_width_pixel <- round(next_crop_width_pixel)
    ...
    if (next_crop_width_pixel < min_allowed_crop_width_pixel || 
        next_crop_height_pixel < min_allowed_crop_height_pixel ||
        next_relative_area_ratio < min_size_ratio) {
      reasons_zh <- c()
      reasons_en <- c()
      if (next_crop_width_pixel < min_allowed_crop_width_pixel || next_crop_height_pixel < min_allowed_crop_height_pixel) {
        reasons_zh <- c(reasons_zh, paste0("下一步的 min(宽, 高)(", next_crop_width_pixel, ", ", next_crop_height_pixel, ") < 最小允许值(", min_pixel_limit, ")"))
        reasons_en <- c(reasons_en, paste0("Next step min(width, height)(", next_crop_width_pixel, ", ", next_crop_height_pixel, ") < min allowed value(", min_pixel_limit, ")"))
      }
      if (next_relative_area_ratio < min_size_ratio) {
        reasons_zh <- c(reasons_zh, paste0("下一步面积比例(", next_relative_area_ratio, ") < 最小比例(", min_size_ratio, ")"))
        reasons_en <- c(reasons_en, paste0("Next step area ratio(", next_relative_area_ratio, ") < min ratio(", min_size_ratio, ")"))
      }
      
      if (length(reasons_zh) > 0 && length(reasons_en) > 0) {
        cat(paste0("停止生成图像 ", crop_sequence_index+1, "，原因：", paste(reasons_zh, collapse = ", "), "\n"))
        cat(paste0("Stop generating image ", crop_sequence_index+1, ", reasons: ", paste(reasons_en, collapse = ", "), "\n"))
      }
      break
    }
    
    # 更新尺寸和序号/Update dimensions and index
    current_crop_width_pixel <- next_crop_width_pixel
    current_crop_height_pixel <- next_crop_height_pixel
    crop_sequence_index <- crop_sequence_index + 1
  }
  
  cat(paste0("完成！已生成 ", crop_sequence_index, " 张图像，保存在 ", output_directory, " 文件夹中\n"))
  cat(paste0("Completed! Generated ", crop_sequence_index, " images saved in ", output_directory, " folder\n"))
}
